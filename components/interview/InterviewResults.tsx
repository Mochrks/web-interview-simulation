...
export default function InterviewResults({
  ...
  if (stageScores.behavioral >= 70)
    strengths.push('Strong communication and behavioral skills');
  ...
  if (stageScores.coding >= 70)
    strengths.push('Excellent coding and problem-solving abilities');
  else weaknesses.push('Practice more coding challenges');

  if (stageScores.final >= 70)
    strengths.push('Good cultural fit and professional presence');
  else
    weaknesses.push('Work on professional communication and company research');
  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-6 space-y-6 md:space-y-8">
      ...
      <div className="score-display animate-slide-up py-6 sm:py-8">
        <div className="text-xs sm:text-sm uppercase font-bold mb-2">
          Overall Score
        </div>
        <div className="text-6xl sm:text-8xl">{Math.round(overallScore)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        <div className="radar-chart-container bg-card p-4 sm:p-6">
          <h3 className="bauhaus-subheading text-lg sm:text-xl mb-4 text-center sm:text-left">
            Skills Assessment
          </h3>
      ...
      <div className="bauhaus-card">
        <h3 className="bauhaus-subheading text-xl sm:text-2xl mb-6 text-center sm:text-left">
          Stage Breakdown
        </h3>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {barData.map((stage) => (
            <div
              key={stage.stage}
              className={`border-4 border-foreground p-3 sm:p-4 ${getScoreColor(stage.score)}`}
            >
              <h4 className="font-bold uppercase text-[10px] sm:text-xs mb-2">
                {stage.stage}
              </h4>
              <div className="text-3xl sm:text-4xl font-black">
                {stage.score}
              </div>
              <div className="text-[10px] sm:text-sm mt-1">
                {getScoreLabel(stage.score)}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        <div className="bauhaus-card bauhaus-accent-blue">
          <h3 className="bauhaus-subheading text-lg sm:text-xl mb-4">
            💪 Strengths
          </h3>
      ...
      <div className="bauhaus-card">
        <h3 className="bauhaus-subheading text-xl sm:text-2xl mb-4">
          💡 Recommendations
        </h3>
        <ul className="space-y-3">
          {recommendations.map((rec, i) => (
            <li key={i} className="flex items-start gap-3">
              <span className="text-xl sm:text-2xl font-bold text-[hsl(var(--bauhaus-red))]">
                {i + 1}.
              </span>
              <span className="text-base sm:text-lg">{rec}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="bauhaus-card">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <Button
            ...
            className="bauhaus-btn bauhaus-accent-blue px-4 py-2 text-sm sm:text-lg"
          >
          ...
          <Button
            ...
            disabled={isExporting}
            className="bauhaus-btn bauhaus-accent-red px-4 py-2 text-sm sm:text-lg"
          >
          ...
          <Button
            ...
            disabled={isExporting}
            className="bauhaus-btn bauhaus-accent-yellow px-4 py-2 text-sm sm:text-lg"
          >
      ...
      <div className="bauhaus-card">
        <div className="flex justify-center">
          <Button
            onClick={onRestart}
            className="bauhaus-btn bauhaus-accent-red text-lg sm:text-xl px-8 sm:px-12 py-4 sm:py-6 w-full sm:w-auto"
          >
            Start New Interview
          </Button>
        </div>
